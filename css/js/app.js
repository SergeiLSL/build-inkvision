(() => {
    ...
    (() => {
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                ...
                if ("email" === formRequiredItem.dataset.required) {
                    ...
                    this.addError(formRequiredItem);
                    error++;
                } else if ("password" === formRequiredItem.dataset.required) {
                    formRequiredItem.value = formRequiredItem.value.replace(" ", "");
                    if (!this.passwordTest(formRequiredItem) || !this.passwordMatch(formRequiredItem)) {
                        this.addError(formRequiredItem);
                        error++;
                    } else this.removeError(formRequiredItem);
                } else if ("text" === formRequiredItem.dataset.required) if ("" === formRequiredItem.value) {
        ...
        let script_all = document.querySelectorAll(".r-all");
        for (let a = 0; a < script_all.length; a++) {
            let radios = script_all[a].querySelectorAll(".r-input");
            let i = 1;
            script_all[a].style.setProperty("--options", radios.length);
            radios.forEach((input => {
                input.setAttribute("data-pos", i);
                input.addEventListener("click", (e => {
                    script_all[a].style.setProperty("--options-active", e.target.getAttribute("data-pos"));
                }));
                i++;
            }));
        }
